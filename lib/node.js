...
function Node(config) {
  ...
  console.log("*core Node() this.configPath: ", this.configPath);
  
  // TODO type check the arguments of config.services
  if (config.services) {
    console.log("  config.services: " + JSON.stringify(config.services));
  ...
  // configurable networks for altcoins to override 'livenet' and 'testnet'
  // { livenet: { ... }, testnet: { ... } }
  this.networkOptions = config.networkOptions;
  this._setNetwork(this.networkOptions);
...
Node.prototype._setNetworkOptions = function(opts) {
  if (opts.livenet) {
    // remove existing network if there, and add configured one
  }
  if (opts.testnet) {
...
Node.prototype.getServiceOrder = function() {
  ...
  console.log("*coin Node.getServiceOrder() names: " + JSON.stringify(names));
  addToStack(names);
...
Node.prototype._startService = function(serviceInfo, callback) {
  ...
  if (serviceInfo.config) {
    //console.log('  with serviceInfo.config ');
    log.info('  with serviceInfo.config ');
    ...
    //console.log('  with empty config ');
    log.info('  with empty config ');
    config = {};
  }

  config.node = this;
  config.name = serviceInfo.name;
  //console.log("  node: loading service " + serviceInfo.name + " with config: \n", config);
  var service = new serviceInfo.module(config);
  //console.log("  instanstated service: ", service);

  // try to pass config since its not getting picked up in service.start()
  //console.log("  after new() service.config = ", service.config);
  if(! service.customConfig ) {
    service.customConfig = config;
    console.log("  set service.customConfig = config");
  }
